...
func main() {
	...
	// Fail fast if required image env vars are unset.
	// OPERATOR_IMAGE: catalog-builder Jobs; MANAGER_IMAGE: manager Deployments; WORKER_IMAGE: worker/cleanup Pods.
	// Missing values surface as cryptic ImagePullBackOff failures at runtime.
	for _, envVar := range []string{"OPERATOR_IMAGE", "MANAGER_IMAGE", "WORKER_IMAGE"} {
		if os.Getenv(envVar) == "" {
			setupLog.Error(nil, "required environment variable is not set", "name", envVar)
			os.Exit(1)
		}
	}
